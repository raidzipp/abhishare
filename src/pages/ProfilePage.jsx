      ...
      <div className="profile-header animate-in">
        <div className="profile-avatar-wrap">
          {profile?.profile_photo_url ? (
            ...
            <span style={{ fontSize: 28, fontWeight: 700, color: 'var(--brand-1)' }}>{getInitial(profile?.full_name)}</span>
          )}
        </div>
        ...
        <Link to="/profile/edit" className="btn btn-outline" style={{ marginTop: 12, display: 'inline-flex' }}>
                ...
                {THEMES.map(t => (
                  <button key={t.key} onClick={() => handleTheme(t.key)}
                    style={{
                      ...
                      border: currentTheme === t.key ? '2px solid var(--brand-1)' : '1px solid var(--border)',
                      background: currentTheme === t.key ? 'var(--brand-glow)' : 'var(--card-bg)',
                      cursor: 'pointer', textAlign: 'center', transition: 'all 0.12s',
                    }}>
                    <div style={{
                      width: '100%', height: 4, borderRadius: 2, marginBottom: 8,
                      background: `linear-gradient(90deg, ${t.colors[0]}, ${t.colors[1]}, ${t.colors[2]})`
                    }} />
                    <div style={{ fontSize: 'var(--text-sm)', fontWeight: currentTheme === t.key ? 600 : 500, color: currentTheme === t.key ? 'var(--brand-1)' : 'var(--text-secondary)' }}>
                      {t.name}
                    </div>
                  </button>
                ))}
